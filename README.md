...
разросшегося `CLAUDE.md`. Приоритет посекционный: секция файла ревью перекрывает `CLAUDE.md` на
ту же тему, остальное по-прежнему берётся из `CLAUDE.md`. Распознаваемые заголовки:

| Секция | Что задаёт |
|---|---|
| «Уровни и зоны» | пороги уровней, security-поверхность и контракты (`max`), зоны ядра (`xhigh`) |
| «Не репортить» | skip-list: классы находок, которые не репортятся (security не подавляется никогда) |
| «Нормы проходов» | уровень и scope проходов 2 и далее; номер прохода — аргумент `pass=N` |
| «Верификационная планка» | требования к доказательству вердиктов (ужесточает, но не отменяет верификацию) |

Остальные секции — обычные правила проекта. После `ReportFindings` skill печатает сводку прогона:
агенты, минуты, токены (сумма `subagent_tokens`; если недоступны — подсказка ввести `/cost`),
источник правил и число подавленных skip-list кандидатов — её переносят в описание PR.
...
С 1.19.0 skill совместим с проектными хуками-гейтами: описание PR с метаданными ревью готовит
файлом и передаёт `/dev-toolkit:cppr` (тот зовёт `gh pr create --body-file`), перед мёржем ждёт
**все** проверки `gh pr checks`, а не только `--required`, и мержит с явным номером PR.
