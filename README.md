...
остальное по-прежнему берётся из `CLAUDE.md`. Распознаются заголовки, **точно** равные
`Уровни и зоны`, `Не репортить`, `Нормы проходов`, `Верификационная планка` (жирный пункт списка
заголовком не считается); прочие секции — обычные правила проекта. Файл ревью, изменённый в самом
ревьюируемом диффе, этот дифф не судит: его skip-list и понижающие уровень правила в таком прогоне
не применяются. Номер прохода — аргумент `pass=N`. Точная механика и ограничения (security skip-list
не подавляет никогда) — раздел «Правила ревью проекта» в `skills/review-code/SKILL.md`.

После `ReportFindings` skill печатает сводку прогона (уровень, target, проход, режим, агенты,
минуты, токены или подсказка ввести `/cost`, источник правил, подавленное skip-list) — её переносят в
описание PR.
...
`xhigh`, при касании security-поверхности или контрактов — `max`) и `/dev-toolkit:cppr`; описание PR
собирает по образцу `cppr`, добавляя метаданные раундов ревью, которые есть только у него.
...
`gh pr create --body-file`; `/dev-toolkit:pr` — так же), перед мёржем ждёт **все** проверки и судит
